...
function M.content(config)
	...
	if config.process_comment_signs == "spaces" then
		local raw_token = vim.iter(comment_tokens.raw):flatten():totable()
		for i, token in ipairs(vim.iter(comment_tokens.escaped):flatten():totable()) do
			content = content:gsub(token, string.rep(" ", #raw_token[i]))
		end
	elseif config.process_comment_signs == "delete" then
		for _, sign in ipairs(vim.iter(comment_tokens.escaped):flatten():totable()) do
